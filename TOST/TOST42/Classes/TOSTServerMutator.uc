// $Id: TOSTServerMutator.uc 566 2004-04-19 15:05:24Z stark $
//----------------------------------------------------------------------------
// Project : TOST
...
// 1.1		+ Added TOSTLogFileFormat (by DiLDoG)
//----------------------------------------------------------------------------
//	DiLDoG sais:
//		Only added var config string TOSTLogFileFormat; + defaultvar
//		in TOSTStatLog Added some functions @ end and changed StartLog()
//		Have fun with it :P
//----------------------------------------------------------------------------
...
var config	string	TOSTLogFileFormat;
...
function	LoadPieces()
{
	...
	for (i=0; i<ArrayCount(Pieces); i++)
	{
		...
		while (Right(Pieces[i], 1) == " ")
        	Pieces[i] = Left(Pieces[i], Len(Pieces[i]) - 1);
...
{
	bHidden=True
	// i'd like to start the Linux-Kernel version numbering:
	// _._.x._ : x==odd internal, x==even release
	TOSTVersion="TOST 4.2.2.0"
	TOSTLogEnabled=True
	TOSTLogFilter=""
