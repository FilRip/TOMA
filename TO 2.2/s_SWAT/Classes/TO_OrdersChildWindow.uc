...
class TO_OrdersChildWindow expands s_SWATWindow;
			...
			for (i=0; i<NumOptions; i++)
			{
				if (B == OptionButtons[i])
				{
					...
					else 
					{
						...
						HideChildren();
						SpeechChild = SpeechWindow(ParentWindow.CreateWindow(class'TO_TargetChildWindow', 100, 100, 100, 100));
						SpeechChild.CurrentType = i+2;
						TO_TargetChildWindow(SpeechChild).Message = OtherOffset[i];
						SpeechChild.FadeIn();
					}
				}
			}
