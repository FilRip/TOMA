class TOMAChrekMasterChunk extends UTMasterCreatureChunk;
...
simulated function ClientExtraChunks()
{
	...
	{
		carc = Spawn(class'tomachrekhead');
		if ( carc != None )
			carc.Initfor(self);
	}
	...
	if ( CarcassAnim != 'Dead5' )
	{
		if ( Level.bHighDetailMode && !Level.bDropDetail )
		{
			...
			if ( FRand() < 0.5 )
			{
				carc = Spawn(class 'tomachrekleg');
				if (carc != None)
					carc.Initfor(self);
			}
		}
		carc = Spawn(class 'tomachrektorso');
		if (carc != None)
			carc.Initfor(self);
		carc = Spawn(class 'tomachrekarm');
		if (carc != None)
			carc.Initfor(self);
	}
