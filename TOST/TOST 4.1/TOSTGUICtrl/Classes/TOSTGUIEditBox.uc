...
class TOSTGUIEditBox expands UWindowDialogControl;
...
function Paint(Canvas Canvas, float X, float Y)
{
	...
	TextSize(Canvas, "A", W, H);
	TextY = (WinHeight - H) / 2;

	TextSize(Canvas, Left(Value, CaretOffset), W, H);
	...
	if(bAllSelected)
	{
		DrawStretchedTexture(Canvas, Offset + 1, TextY, W, H, Texture'UWindow.WhiteTexture');

		// Invert Colors
		Canvas.DrawColor.R = 255 ^ Canvas.DrawColor.R;
		Canvas.DrawColor.G = 255 ^ Canvas.DrawColor.G;
		Canvas.DrawColor.B = 255 ^ Canvas.DrawColor.B;
	}

	ClipText(Canvas, Offset + 1, TextY,  Value);
	...
	if(bShowCaret)
		ClipText(Canvas, Offset + W - 1, TextY, "|");
}
