class AssaultSwatWindow extends s_swatwindow;
...
function Notify (UWindowWindow B, byte E)
{
	...
	switch (E)
	{
		...
		switch (B)
		{
			...
			if (!AssaultPlayer(GetPlayerOwner()).bNotPlaying)
			{
			...
			HideChildren();
			CurrentType=SpeechButton(B).Type;
			SpeechChild=SpeechWindow(CreateWindow(Class'AssaultChildChangeTeamWindow',100,100,100,100));
			SpeechChild.FadeIn();
			break;
