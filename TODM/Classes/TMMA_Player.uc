class TMMA_Player extends TMPlayer;
...
{
	local TMMA_Mutator MA_Mutator;

	Super.PostBeginPlay();
	/*
	if ( Level.NetMode == NM_StandAlone )
	{
		foreach AllActors(Class'TMMA_Mutator',MA_Mutator)
		{
...
function PostRender (Canvas Canvas)
{
	...
	if ( s_HUD(myHUD) != None && s_HUD(myHUD).bToggleBuymenu )
	{
		if ( !bMABuymenu )
		{
			s_HUD(MyHud).UserInterface.TOUI_Tool_AddTab(137,Class'TMMA_BuyMenu');
			bMABuymenu=True;
		}
...
function MA_DrawNightVision (Canvas Canvas)
{
	if ( MA_NightVision && bOwnsNV && bNVon )
	{
		...
		if ( MA_Light == None )
		{
			MA_Light=Spawn(Class'TMMA_Light',self,,Location);
		}
	}
