...
class s_ChildTerroristTeam expands s_SWATWindow;
...
function int CountTerroristPlayers()
{
	...
	while (i < 32)
	{
		if (class'TOPModels.TO_ModelHandler'.static.IsATerrmodel(i))
		{
			Playernumber[num] = i;
			num++;
		}
...
function Created()
{
	...
	{
		OptionButtons[i].Text = class'TOPModels.TO_ModelHandler'.default.ModelName[Playernumber[i]];
	}
	...
	switch (E)
	{
		...
		case DE_Click:
			...
			for (i=0;i<NumOptions;i++)
			{
				...
				{
					s_Player(GetPlayerOwner()).s_ChangeTeam(Playernumber[i], 0, true);
				}
			}
			if (B == TopButton)
			{
