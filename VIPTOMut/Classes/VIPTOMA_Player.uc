class VIPTOMA_Player extends VIPTO_Player;
...
function PostRender (Canvas Canvas)
{
	...
	if ( s_HUD(myHUD) != None && s_HUD(myHUD).bToggleBuymenu )
	{
		if ( !bMABuymenu )
		{
			s_HUD(MyHud).UserInterface.TOUI_Tool_AddTab(137,Class'VIPTOMA_BuyMenu');
			bMABuymenu=True;
		}
...
function MA_DrawNightVision (Canvas Canvas)
{
	if ( MA_NightVision && bOwnsNV && bNVon )
	{
		...
		if ( MA_Light == None )
		{
			MA_Light=Spawn(Class'VIPTOMA_Light',self,,Location);
		}
	}
