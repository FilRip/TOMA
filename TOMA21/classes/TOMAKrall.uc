class TOMAKrall extends TOMAScriptedPawn;
...
function PlayTakeHit(float tweentime, vector HitLoc, int damage)
{
	...
	Velocity.Z*=1.3;
	rep=Spawn(class'TOMALeglessKrall');
	rep.InitFor(self);
	destroy();
}
...
function ThrowDice()
{
	...
	d1 = TOMAKrall(TeamLeader).Toy1;
	d2 = TOMAKrall(TeamLeader).Toy2;
...
function GrabDice()
{
	if ( TOMAKrall(TeamLeader).Toy1 == None )
		TOMAKrall(TeamLeader).Toy1 = Spawn(class'Dice');
	if ( TOMAKrall(TeamLeader).Toy2 == None )
		TOMAKrall(TeamLeader).Toy2 = Spawn(class'Dice');

	TOMAKrall(TeamLeader).Toy1.bHidden = True;
	TOMAKrall(TeamLeader).Toy2.bHidden = True;
}
...
function PlayWaiting()
{
	...
	if ( bDicePlayer )
	{
		...
		if ( bHasDice )
		{
			...
			{
				bHasDice = false;
				TOMAKrall(TeamLeader).bHeldDice = false;
			}
			else if ( FRand() < 0.8 )
			{
		...
		if ( TOMAKrall(TeamLeader).bHeldDice || ( FRand() < 0.65) )
		{
			if ( decision < 0.8 )
				LoopAnim('Breath2', animspeed, 0.5);
		...
		TOMAKrall(TeamLeader).bHeldDice = True;
		bHasDice = True;
