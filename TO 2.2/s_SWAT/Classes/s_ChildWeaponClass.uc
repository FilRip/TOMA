...
class s_ChildWeaponClass expands s_SWATWindow;


var		int		 	OptionOffset;
var		int			MinOptions;

var		string	ClassName[4];
...
function Created()
{
	...
	{
		OptionButtons[i].Text = ClassName[i];
	}
		...
		case DE_Click:
			...
			switch (B)
			{
				case OptionButtons[0]:					
					...
					s_SwatWindow(ParentWindow).Option1 = 5; // WeaponClass to display
					s_SwatWindow(ParentWindow).Option2 = -1;	// ChildWindow TOP
					SpeechChild = SpeechWindow(ParentWindow.CreateWindow(class's_SWAT.s_ChildWeaponSelect', 100, 100, 100, 100));
					SpeechChild.FadeIn();
					break;
				...
				case OptionButtons[1]:
					...
					HideChildren();
					CurrentType = SpeechButton(B).Type;
					s_SwatWindow(ParentWindow).Option1 = 1; // WeaponClass to display
					s_SwatWindow(ParentWindow).Option2 = 0;	// ChildWindow TOP
					SpeechChild = SpeechWindow(ParentWindow.CreateWindow(class's_SWAT.s_ChildWeaponSelect', 100, 100, 100, 100));
					SpeechChild.FadeIn();
					break;
				...
				case OptionButtons[2]:
					...
					HideChildren();
					CurrentType = SpeechButton(B).Type;
					s_SwatWindow(ParentWindow).Option1 = 2; // WeaponClass to display
					s_SwatWindow(ParentWindow).Option2 = 1;	// ChildWindow TOP
					SpeechChild = SpeechWindow(ParentWindow.CreateWindow(class's_SWAT.s_ChildWeaponSelect', 100, 100, 100, 100));
					SpeechChild.FadeIn();
					break;
				...
				case OptionButtons[3]:
					...
					HideChildren();
					CurrentType = SpeechButton(B).Type;
					s_SwatWindow(ParentWindow).Option1 = 3; // WeaponClass to display
					s_SwatWindow(ParentWindow).Option2 = 2;	// ChildWindow TOP
					SpeechChild = SpeechWindow(ParentWindow.CreateWindow(class's_SWAT.s_ChildWeaponSelect', 100, 100, 100, 100));
					SpeechChild.FadeIn();
					break;
			}
...
defaultproperties
{
     ClassName(0)="Grenades"
     ClassName(1)="Pistols"
     ClassName(2)="SMGs & Shots"
     ClassName(3)="Rifles"
