class MA_Player extends s_Player_T;
...
{
	local MA_Mutator MA_Mutator;

	Super.PostBeginPlay();
	/*
	if ( Level.NetMode == NM_StandAlone )
	{
		foreach AllActors(Class'MA_Mutator',MA_Mutator)
		{
...
function PostRender (Canvas Canvas)
{
	...
	if ( s_HUD(myHUD) != None && s_HUD(myHUD).bToggleBuymenu )
	{
		if ( !bMABuymenu )
		{
			s_HUD(MyHud).UserInterface.TOUI_Tool_AddTab(137,Class'MA_BuyMenu');
			bMABuymenu=True;
		}
...
function MA_DrawNightVision (Canvas Canvas)
{
	if ( MA_NightVision && bOwnsNV && bNVon )
	{
		...
		if ( MA_Light == None )
		{
			MA_Light=Spawn(Class'MA_Light',self,,Location);
		}
	}
