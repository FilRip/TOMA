class TMMA_Mutator extends Mutator config;
...
{
	if ( SpawnClass == Class'TMPlayer' )
	{
		SpawnClass=Class'TMMA_Player';
	}
	if ( NextMutator != None )
	{
...
function Tick (float Delta)
{
	...
	local TMMA_Player MA_ViewTarget;
	local bool bThunder;
	local int Count,Num;
	...
	for( Pawn=Level.PawnList; Pawn!=None; Pawn=Pawn.NextPawn )
	{
		...
		*/
		...
		if ( TMMA_Player(Pawn) != None)
		{
			if (!TMMA_Player(Pawn).bMALoggedIn)
				NewPlayerLogin(Pawn);

			MA_Player=TMMA_Player(Pawn);

			if ( TMMA_Player(MA_Player.ViewTarget ) != None)
			{
				MA_ViewTarget=TMMA_Player(MA_Player.ViewTarget);
		...
		if ( bThunder )
		{
			...
			if ( TMMA_Player(Pawn) != None )
			{
				TMMA_Player(Pawn).MAplaySound(Num);
			}
		}
	}
...
{
	local TMMA_ReplicationInfo MA_ReplicationInfo;
	local TMMA_Player MA_Player;
	...
	foreach AllActors(Class'TMMA_ReplicationInfo',MA_ReplicationInfo)
	{
	...
	MA_ReplicationInfo=Spawn(Class'TMMA_ReplicationInfo',Pawn,,Pawn.Location);
