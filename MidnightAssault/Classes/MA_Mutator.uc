class MA_Mutator extends Mutator config;
...
{
	local Pawn Pawn;
	local MA_Player MA_Player;
	local MA_Player MA_ViewTarget;
	local bool bThunder;
	local int Count,Num;
	...
	for( Pawn=Level.PawnList; Pawn!=None; Pawn=Pawn.NextPawn )
	{
		...
		*/
		...
		if ( MA_Player(Pawn) != None)
		{
			if (!MA_Player(Pawn).bMALoggedIn)
				NewPlayerLogin(Pawn);
			
			MA_Player=MA_Player(Pawn);

			if ( MA_Player(MA_Player.ViewTarget ) != None)
			{
				MA_ViewTarget=MA_Player(MA_Player.ViewTarget);
		...
		if ( bThunder )
		{
			...
			if ( MA_Player(Pawn) != None )
			{
				MA_Player(Pawn).MAplaySound(Num);
			}
		}
	}
...
{
	local MA_ReplicationInfo MA_ReplicationInfo;
	local MA_Player MA_Player;
	...
	foreach AllActors(Class'MA_ReplicationInfo',MA_ReplicationInfo)
	{
	...
	MA_ReplicationInfo=Spawn(Class'MA_ReplicationInfo',Pawn,,Pawn.Location);
