...
function bool SuccessfulGame() { return true; }
function bool ShouldRespawn(Actor Other) {return false; }
function float PlaySpawnEffect(inventory Inv) {	return 0.3; }
function PlayTeleportEffect( actor Incoming, bool bOut, bool bSound) {}


function RegisterDamageMutator(Mutator M) 
{	
	Log("TO_DeathMatchPlus::RegisterDamageMutator - M:"@M);
	DamageMutator = None; 
}

function RegisterMessageMutator(Mutator M) 
{
	Log("TO_DeathMatchPlus::RegisterMessageMutator - M:"@M);
...
event InitGame( string Options, out string Error )
{
	...
	DamageMutator = None;
	MessageMutator = Spawn(class's_SWAT.TO_MessageMutator');
...
*/
		...
		// do not restart players when match starts
/*
	for ( P = Level.PawnList; P!=None; P=P.nextPawn )
		if ( P.bIsPlayer && !P.IsA('PlayerPawn') )
		{
...
function Timer()
{
	...
	if ( Level.NetMode != NM_StandAlone )
	{
		while ( NeedPlayers() )
		{
			//log("TO_DeathMatchPlus::Timer - AddBot");
			AddBot();
		}
	}
/*
...
{
	//log("TO_DeathMatchPlus::ForceAddBot");

	// add bot during gameplay
	if ( Level.NetMode != NM_Standalone )
	{
		MinPlayers = Max(MinPlayers+1, NumPlayers + NumBots + 1);
		MinPlayers = Min(MinPlayers, 32);
		if ( MinPlayers < 32 )
			AddBot();
	}
	else
	{
		if ( NumPlayers + NumBots < 32 )
			AddBot();
	}
}
...
function Logout(pawn Exiting)
{
	...
	{
		//log("TO_DeathMatchPlus::Logout - AddBot");
		RemainingBots++;
	}
}
...
{
	return (!bGameEnded && (NumPlayers + NumBots < Min(MinPlayers, 32)));
}
