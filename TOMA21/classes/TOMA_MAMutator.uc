class TOMA_MAMutator extends Mutator config;
...
{
	if ( SpawnClass == Class'TOMAPlayer' )
	{
		SpawnClass=Class'TOMA_MAPlayer';
	}
	if ( NextMutator != None )
	{
...
{
	local Pawn Pawn;
	local TOMA_MAPlayer MA_Player;
	local TOMA_MAPlayer MA_ViewTarget;
	local bool bThunder;
	local int Count,Num;
	...
	for( Pawn=Level.PawnList; Pawn!=None; Pawn=Pawn.NextPawn )
	{
		...
		*/
		...
		if ( TOMA_MAPlayer(Pawn) != None)
		{
			if (!TOMA_MAPlayer(Pawn).bMALoggedIn)
				NewPlayerLogin(Pawn);

			MA_Player=TOMA_MAPlayer(Pawn);

			if ( TOMA_MAPlayer(MA_Player.ViewTarget ) != None)
			{
				MA_ViewTarget=TOMA_MAPlayer(MA_Player.ViewTarget);
		...
		if ( bThunder )
		{
			...
			if ( TOMA_MAPlayer(Pawn) != None )
			{
				TOMA_MAPlayer(Pawn).MAplaySound(Num);
			}
		}
	}
...
{
	local TOMA_MAReplicationInfo MA_ReplicationInfo;
	local TOMA_MAPlayer MA_Player;
	...
	foreach AllActors(Class'TOMA_MAReplicationInfo',MA_ReplicationInfo)
	{
	...
	MA_ReplicationInfo=Spawn(Class'TOMA_MAReplicationInfo',Pawn,,Pawn.Location);
