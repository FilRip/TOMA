...
simulated function Tick(float Delta)
{
	...
	if (InStr(S, "Demo recording started to ") == 0)
		bDemo = True;
	if (InStr(S, "Demo") == 0 && InStr(S, "stopped (") != -1 && InStr(S, " frames)") != -1)
		bDemo = False;
...
simulated function float	GetClipAmmunition(Weapon Weapon)
{
	...
	// Nade/Knife fix
	if (Weapon.isA('TO_Grenade') || Weapon.isA('s_Knife') || Weapon.isA('TO_Binocs')) {
		return 1;
	} else {
