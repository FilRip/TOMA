...
// 1.2		+ Added servername in quicksettings
//----------------------------------------------------------------------------
...
//var config	string	StartTime[10];
...
function		GetSettings(TOSTPiece Sender)
{
	...
	Params.Param4 = ((SG.TimeLimit & 0xFFFF) << 16) + ((SG.RoundLimit & 0xFF) << 8) + (SG.RoundDuration & 0xFF)$";"$
    				...
    				int(SG.FriendlyFireScale*100);
    for (i=0; i<10; i++)
    	Params.Param4 = Params.Param4$";"$TOST.SA.Mutators[i];

 	Params.Param4 = Params.Param4$";servername";
	...
	if (s != "")
	{
		...
		if (s != "")
		{
			...
			if (s != "")
			{
				...
				if (j != -1)
				{
					SG.FriendlyFireScale = float(Left(s, j))/100;
					s = Mid(s, j+1);
				} else {
					SG.FriendlyFireScale = float(s)/100;
					s = "";
				}
				GRI.friendlyfirescale = SG.FriendlyFireScale;
				...
				if (s != "")
				{
					...
					if (j != -1)
					{
						temp = ReplaceText(Left(s, j),Chr(254),Chr(42));
						s = Mid(s, j+1);
					} else	{
						temp = ReplaceText(s,Chr(254),Chr(42));
						s = "";
					}
					if (temp != "" && temp != "servername")
					{
						GRI.ServerName = temp;
						class'Engine.GameReplicationInfo'.Default.ServerName = temp;
