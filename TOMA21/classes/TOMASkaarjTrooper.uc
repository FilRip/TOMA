...
// SkaarjTrooper.
//=============================================================================
class TOMASkaarjTrooper extends TOMASkaarj;
...
auto state Startup
{
	function BeginState()
	{
		Super.BeginState();
		bIsPlayer=true;
		if ((WeaponType!=None) && (TOMAMod(Level.Game).MonstersCanHaveWeapon))
		{
			bIsPlayer=true;
			if (TOMAMod(Level.Game).bRandomWeaponsForMonsters)
                WeaponType=class<Weapon>(DynamicLoadObject(class'TOMAWeaponsMonsters'.Default.WeaponStr[Rand(10)], class'Class'));
            else if (TOMAMod(Level.Game).WeaponNumberForMonster<10)
                WeaponType=class<Weapon>(DynamicLoadObject(class'TOMAWeaponsMonsters'.Default.WeaponStr[TOMAMod(Level.Game).WeaponNumberForMonster], class'Class'));
			myWeapon=Spawn(WeaponType);
			if (s_Weapon(myWeapon)!=None) s_Weapon(myWeapon).SetRemainingAmmo(5,30,false);
			myWeapon.SetLocation(self.location);
			if (myWeapon!=None)
	...
	{
		Super.SetHome();
		if ((myWeapon!=None) && (TOMAMod(Level.Game).MonstersCanHaveWeapon))
			myWeapon.Touch(self);
	}
Begin:
	StartRoaming();
...
function bool CanFireAtEnemy()
{
	...
	if (Weapon==None)
		return false;
	if ((s_Weapon(Weapon).RemainingClip==0) && (s_Weapon(Weapon).ClipAmmo==0)) return false;
	GetAxes(Rotation,X,Y,Z);
...
defaultproperties
{
	WeaponType=class's_swat.s_ak47'
	LungeDamage=20
	SpinDamage=15
	ClawDamage=10
	...
	Mass=125
	Buoyancy=125
	NameOfMonster="SkaarjTrooper"
	sshot1="TOMATex21.Sshot.SkaarjTrooper"
}
