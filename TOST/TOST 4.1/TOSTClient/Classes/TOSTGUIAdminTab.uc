...
var localized string		TextButtonProtectSrv, TextHintProtectSrvButton;

var TOSTGUIBaseButton		ButtonClose, ButtonGameTab;
var TOSTGUIBaseButton		ButtonSwitchLvl, ButtonSkipLvl, ButtonSetNextLvl;
var TOSTGUIBaseButton		ButtonKick, ButtonBan, ButtonChgTeams, ButtonMkTeams;
var TOSTGUITextListbox		BoxLevels, BoxPlayers;
var TOSTGUIBaseButton		ButtonAdminReset, ButtonEndRound, ButtonProtectSrv;
...
simulated function Created ()
{
	...
	ButtonProtectSrv = TOSTGUIBaseButton(CreateWindow(class'TOSTGUIBaseButton', 0, 0, WinWidth, WinHeight));
	ButtonProtectSrv.Text = TextButtonProtectSrv;
	ButtonProtectSrv.OwnerTab = self;
...
simulated function Notify (UWindowDialogControl control, byte event)
{
	...
	if (control == ButtonGameTab)
	{
		if (event == DE_Click)
		{
			// OwnerInterface.ToggleTab(201 /*UIT_GAME*/);
	...
	else if (control == ButtonMkTeams)
	{
		...
		{
			OwnerPlayer.ConsoleCommand("mkteams true");
			ClearUp();
		}
		else if (event == DE_MouseMove)
		{
	...
	else if (control == ButtonKick)
	{
		if (event == DE_Click)
		{
			...
			{
				if (BoxPlayers.IsSelectedByIndex(i))
					OwnerPlayer.ConsoleCommand("SAPKick "@BoxPlayers.GetData(i));
			}
			ClearUp();
		}
		else if (event == DE_RClick)
		{
			...
			{
				if (BoxPlayers.IsSelectedByIndex(i))
					OwnerPlayer.ConsoleCommand("SAXPKick "@BoxPlayers.GetData(i));
			}
			ClearUp();
		}
		else if (event == DE_MouseMove)
	...
	else if (control == ButtonBan)
	{
		if (event == DE_Click)
		{
			...
			{
				if (BoxPlayers.IsSelectedByIndex(i))
					OwnerPlayer.ConsoleCommand("SAPKickban "@BoxPlayers.GetData(i));
			}
			ClearUp();
		}
		else if (event == DE_RClick)
		{
			...
			{
				if (BoxPlayers.IsSelectedByIndex(i))
					OwnerPlayer.ConsoleCommand("SAPTempKickBan "@BoxPlayers.GetData(i));
			}
			ClearUp();
		}
		else if (event == DE_MouseMove)
	...
	// ProtectSrv
	else if (control == ButtonProtectSrv)
	{
		if (event == DE_Click)
		{
			OwnerPlayer.ConsoleCommand("ProtectSrv");
			ClearUp();
		}
		else if (event == DE_RClick)
		{
...
simulated function Setup(Canvas Canvas)
{
	...
	ButtonProtectSrv.WinLeft = t;
	ButtonProtectSrv.WinTop = BoxLevels.WinTop + BoxLevels.WinHeight + ButtonSwitchLvl.WinHeight + 2*ButtonSkipLvl.WinHeight + 6*Padding[Resolution];
	ButtonProtectSrv.SetWidth(Canvas, xw);
...
defaultproperties
{
	...
	TextHintEndRoundButton="End the round"
 	TextButtonProtectSrv="pw-protect server"
	TextHintProtectSrvButton="Protect the server for a given period of time with a random password"
