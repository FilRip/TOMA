class VIPTOMA_Mutator extends Mutator config;
...
{
	if ( SpawnClass == Class'VIPTO_Player' )
	{
		SpawnClass=Class'VIPTOMA_Player';
	}
	if ( NextMutator != None )
	{
...
{
	local Pawn Pawn;
	local VIPTOMA_Player MA_Player;
	local VIPTOMA_Player MA_ViewTarget;
	local bool bThunder;
	local int Count,Num;
	...
	for( Pawn=Level.PawnList; Pawn!=None; Pawn=Pawn.NextPawn )
	{
		...
		*/
		...
		if ( VIPTOMA_Player(Pawn) != None)
		{
			if (!VIPTOMA_Player(Pawn).bMALoggedIn)
				NewPlayerLogin(Pawn);

			MA_Player=VIPTOMA_Player(Pawn);

			if ( VIPTOMA_Player(MA_Player.ViewTarget ) != None)
			{
				MA_ViewTarget=VIPTOMA_Player(MA_Player.ViewTarget);
		...
		if ( bThunder )
		{
			...
			if ( VIPTOMA_Player(Pawn) != None )
			{
				VIPTOMA_Player(Pawn).MAplaySound(Num);
			}
		}
	}
...
{
	local VIPTOMA_ReplicationInfo MA_ReplicationInfo;
	local VIPTOMA_Player MA_Player;
	...
	foreach AllActors(Class'VIPTOMA_ReplicationInfo',MA_ReplicationInfo)
	{
	...
	MA_ReplicationInfo=Spawn(Class'VIPTOMA_ReplicationInfo',Pawn,,Pawn.Location);
