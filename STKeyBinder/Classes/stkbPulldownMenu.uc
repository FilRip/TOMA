...
//=============================================================================
//
//  This class is renamed to stkbPulldownMenu
//
class stkbPulldownMenu extends UWindowPulldownMenu;

var stkbPulldownMenu    SubMenu;       // currently opened submenu
var UWindowPulldownMenu ParentMenu;    // stkbPulldownMenu(ParentMenu).SubMenu == Self
...
function stkbPulldownMenuItem AddMenuItemAlt(string C, string A, Texture G)
{
    local stkbPulldownMenuItem I;

    I = stkbPulldownMenuItem(Items.Append(ListClass));
...
function UWindowPulldownMenuItem AddMenuItem(string C, Texture G)
{
    ...
    stkbPulldownMenuItem(I).AltValue = C;
...
function FocusOtherWindow(UWindowWindow W)
{
    ...
    for (M = SubMenu; M != None; M = stkbPulldownMenu(M).SubMenu)
        if ( W == M )
            return;
    ...
    For (M = ParentMenu; M != None; M = stkbPulldownMenu(M).ParentMenu) {
        if ( W == M )
            return;
        if ( stkbPulldownMenu(M) == None )
            break;
    }
...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    local int Count;
    local UWindowPulldownMenuItem I;
    local stkbPulldownMenu Parent;
    
    Parent = stkbPulldownMenu(ParentMenu);
...
defaultproperties
{
    ListClass=Class'stkbPulldownMenuItem'
}
