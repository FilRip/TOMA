...
class TOSTGrenade extends TOSTWeaponNoRecoilBug;
...
var int NadeMode, MinNadeMode, MaxNadeMode;
var bool NadeModeEnabled;
...
simulated function bool ClientAltFire( float Value )
{
	...
	NadeMode++;
	if ( NadeMode > MaxNadeMode )
		NadeMode = MinNadeMode;

	if ( Level.NetMode == NM_Client )
	{
		PlaySound(Sound'scopezoom', SLOT_None);
		Pawn(Owner).ReceiveLocalizedMessage(Class'TOSTWeaponMessages',NadeMode);
	}
	return true;
}
...
state ServerHoldGrenade
{
	...
	function DropFrom(vector StartLocation)
	{
		...
		{
			DropGrenade();
//			fixed later to avoid sell hack
		//	FinishGrenade();
		}
		else
...
state ServerThrowingGrenade
{
	...
	//fct trying to drop if player gets killed
	function DropFrom(vector StartLocation)
	{
		if (pawn(owner).Health <= 0)
		{
			DropGrenade();
			// fixed later to avoid hack sell
			//FinishGrenade();
		}
		else
			Super.DropFrom( StartLocation );
...
simulated function ThrowGrenade()
{
	...
	if ( NadeModeEnabled )
	{
		g.ExpTiming = NadeMode - ( 9 - NadeMode ) - Power * ReduceFuse;
		g.LifeSpan = NadeMode;
	}
	else
	{
...
simulated function DropGrenade()
{
	...
	//G.ExpTiming = DropFuse; --> fix this stupid shorter time
	if ( NadeModeEnabled )
	{
		g.ExpTiming = NadeMode - ( 9 - NadeMode ) - Power * ReduceFuse;
		g.LifeSpan = NadeMode;
...
function GrenadeThrown()
{
	...
	//try to fix sell when throwed bug
	FinishGrenade();
}
