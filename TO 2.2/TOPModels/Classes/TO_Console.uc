...
//class TO_Console extends UTConsole;
...
event bool KeyEvent( EInputKey Key, EInputAction Action, FLOAT Delta )
{
	...
	if ( TO_SysPlayer(Viewport.Actor) != None )
		TO_SysPlayer(Viewport.Actor).xxStopTracebot();

	k = Key;
/*
	// Ugly hack for the team selection screen
	if ( Root.GetPlayerOwner().PlayerReplicationInfo.bWaitingPlayer )
		bLocked = true;
	else
		bLocked = false;
*/
	if ( k == ConsoleKey )
	{
		if ( Action == IST_Press )
		{
			//log("TO_Console::KeyEvent - console pressed - bLocked:"@bLocked@" - bWaitingPlayer:"@Root.GetPlayerOwner().PlayerReplicationInfo.bWaitingPlayer);
			if ( bLocked || Root.GetPlayerOwner().PlayerReplicationInfo.bWaitingPlayer )
				return true;

			//log("TO_Console::KeyEvent - console toggle");
...
state UWindow
{
	...
	event Tick( float Delta )
	{
		...
		{
			MenuSong = Music(DynamicLoadObject("seeker.seeker", class'Music'));
...
{
	//if ( Root.GetPlayerOwner().IsA('TO_SysPlayer') )
	//	TO_SysPlayer(Root.GetPlayerOwner()).NotifyLevelChange();
...
function UsePress()
{
	local	TO_SysPlayer	P;

	P = TO_SysPlayer(Root.GetPlayerOwner());
...
exec function ShowObjectives()
{
	...
	foreach Root.GetPlayerOwner().AllActors(class'GameReplicationInfo', GRI)
	{
		...
		if ( ClassIsChildOf(GameClass, AssaultClass) )
		{
			...
			TO_RootWindow(Root).MenuBar.HideWindow();
			LaunchUWindow();
			Root.CreateWindow(class<UWindowWindow>(DynamicLoadObject("TOSystem.TO_InGameObjectives", class'Class')), 100, 100, 100, 100);
		}
	}
}
...
event ConnectFailure( string FailCode, string URL )
{
	...
	if(FailCode == "NEEDPW" || FailCode == "WRONGPW")
	{
		...
		bQuickKeyEnable = True;
		LaunchUWindow();
		W = TO_PasswordWindow(Root.CreateWindow(class'TO_PasswordWindow', 100, 100, 100, 100));
		TO_PasswordCW(W.ClientArea).URL = URL;
	}
}
...
exec function MenuCmd(int Menu, int Item)
{
	...
	if(!bCreatedRoot) 
		CreateRootWindow(None);
	TO_RootWindow(Root).MenuBar.MenuCmd(Menu, Item);
}
...
function StartTimeDemo()
{
	...
	bTimeDemoIsEntry =		Viewport.Actor.Level.Game != None
						...
						&&	!(Left(Viewport.Actor.Level.GetLocalURL(), 9) ~= "to-logo-m");
}
