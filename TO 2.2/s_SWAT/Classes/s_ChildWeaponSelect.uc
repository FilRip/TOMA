//=============================================================================
// s_ChildWeaponSelect
//=============================================================================
//
// Tactical Ops
// - http://www.tactical-ops.net
//
// Source code rights:
// Copyright (C) 2000/2001 Laurent "SHAG" Delayen and Benoit "JAH" Delayen
//=============================================================================

class s_ChildWeaponSelect expands s_SWATWindow;

 
var		int			OptionOffset;
var		int			MinOptions;
var		byte		WepClass;
var		string	WepName[10], PreName;
var		int			WepCode[10];


///////////////////////////////////////
// ListWeapons
///////////////////////////////////////
// Maybe enhance later to sort by price.

function int ListWeapons()
{
	local	int idx, i, j, tmpprice, curprice;
	local	class<s_Weapon>	W;
	local	bool	bContinue;

	i = 0;
	tmpprice = 0;
	bContinue = true;

	//idx =	FindLowestPrice( tmpprice );

	while ( bContinue )
	{
		bContinue = false;
		
		// loop check
		i++;
		if ( i > 10 )
			break;

		if ( tmpprice == 0 )
			idx = FindLowestPrice();
		else
			idx = FindBestWeaponPrice( tmpprice );

		if ( idx != -1 )
		{
			W = class<s_Weapon>(DynamicLoadObject( class'TOModels.TO_WeaponsHandler'.default.WeaponStr[idx], class'Class' ));
			WepCode[j] = idx;
			WepName[j] = PreName$W.default.Price@class'TOModels.TO_WeaponsHandler'.default.WeaponName[idx];
			bContinue = true;
			tmpprice = W.default.price;
			j++;
		}
	}

	return j;
}


///////////////////////////////////////
// FindLowestPrice
///////////////////////////////////////

function	int	FindLowestPrice()
{
	local	int i, bestidx, bestprice;
	local	class<s_Weapon>	W;

	bestprice = 0;

	for (i=0; i <= class'TOModels.TO_WeaponsHandler'.default.NumWeapons; i++)
	{
		if ( (class'TOModels.TO_WeaponsHandler'.default.WeaponStr[i] != "")
			&& (class'TOModels.TO_WeaponsHandler'.static.IsTeamMatch(GetPlayerOwner(), i) == true) ) 
		{
			W = class<s_Weapon>(DynamicLoadObject( class'TOModels.TO_WeaponsHandler'.default.WeaponStr[i], class'Class' ));
			
			if ( (W.default.WeaponClass == WepClass) && ((bestprice == 0) || (W.default.price < bestprice)) )
			{
				bestprice = W.default.price;
				bestidx = i;
			}
		}
	}

	return bestidx;
}


///////////////////////////////////////
// FindBestPrice
///////////////////////////////////////

function	int	FindBestWeaponPrice( int price )
{
	local	int i, bestidx, bestprice, bestecartwoot;
	local	class<s_Weapon>	W;

	bestprice = price;
	bestecartwoot = 20000;

	for (i=0; i <= class'TOModels.TO_WeaponsHandler'.default.NumWeapons; i++)
	{
		if ( (class'TOModels.TO_WeaponsHandler'.default.WeaponStr[i] != "")
			&& (class'TOModels.TO_WeaponsHandler'.static.IsTeamMatch(GetPlayerOwner(), i) == true) ) 
		{
			W = class<s_Weapon>(DynamicLoadObject( class'TOModels.TO_WeaponsHandler'.default.WeaponStr[i], class'Class' ));
			
			if ( (W.default.WeaponClass == WepClass) && (price < W.default.price) )
			{
				if ( (W.default.price - price) < bestecartwoot )
				{
					bestecartwoot = W.default.price - bestprice;
					bestprice = W.default.price;
					bestidx = i;
				}
			}
		}
	}

	if ( price == bestprice )
		return -1;

	//price = bestprice;

	return bestidx;
}


///////////////////////////////////////
// Created
///////////////////////////////////////

function Created()
{
	local int i;
	local int W, H;
	local float XMod, YMod;

	W = Root.WinWidth / 4;
	H = W;

	if(W > 256 || H > 256)
	{
		W = 256;
		H = 256;
	}

	XMod = 4*W;
	YMod = 3*H;

	CurrentType = SpeechWindow(ParentWindow).CurrentType;
	WepClass =  s_SwatWindow(ParentWindow).Option1;
	NumOptions = ListWeapons();

	Super.Created();

	for (i=0; i<NumOptions; i++)
	{
		OptionButtons[i].Text = WepName[i];
	}

	TopButton.OverTexture = texture'OrdersTopArrow';
	TopButton.UpTexture = texture'OrdersTopArrow';
	TopButton.DownTexture = texture'OrdersTopArrow';
	TopButton.WinLeft = 0;
	BottomButton.OverTexture = texture'OrdersBtmArrow';
	BottomButton.UpTexture = texture'OrdersBtmArrow';
	BottomButton.DownTexture = texture'OrdersBtmArrow';
	BottomButton.WinLeft = 0;

	MinOptions = Min(8,NumOptions);

	WinTop = (196.0/768.0 * YMod) + (32.0/768.0 * YMod)*(CurrentType + s_SwatWindow(ParentWindow).Option2);
	WinLeft = 512.0/1024.0 * XMod;
	WinWidth = 256.0/1024.0 * XMod;
	WinHeight = (32.0/768.0 * YMod)*(MinOptions+2);

	SetButtonTextures(0, True, False);
}


///////////////////////////////////////
// BeforePaint
///////////////////////////////////////

function BeforePaint(Canvas C, float X, float Y)
{
	local int W, H;
	local float XWidth, YHeight, XMod, YMod, XPos, YPos, YOffset, BottomTop, XL, YL;
	local color TextColor;
	local int i;

	Super(NotifyWindow).BeforePaint(C, X, Y);

	W = Root.WinWidth / 4;
	H = W;

	if(W > 256 || H > 256)
	{
		W = 256;
		H = 256;
	}

	XMod = 4*W;
	YMod = 3*H;

	XWidth = 256.0/1024.0 * XMod;
	YHeight = 32.0/768.0 * YMod;

	TopButton.SetSize(XWidth, YHeight);
	TopButton.WinTop = 0;
	TopButton.MyFont = class'UTLadderStub'.Static.GetBigFont(Root);
	if (OptionOffset > 0)
		TopButton.bDisabled = False;
	else
		TopButton.bDisabled = True;

	for(i=0; i<OptionOffset; i++)
	{
		OptionButtons[i].HideWindow();
	}
	for(i=OptionOffset; i<MinOptions+OptionOffset; i++)
	{
		OptionButtons[i].ShowWindow();
		OptionButtons[i].SetSize(XWidth, YHeight);
		OptionButtons[i].WinLeft = 0;
		OptionButtons[i].WinTop = (32.0/768.0*YMod)*(i+1-OptionOffset);
	}
	for(i=MinOptions+OptionOffset; i<NumOptions; i++)
	{
		OptionButtons[i].HideWindow();
	}

	BottomButton.SetSize(XWidth, YHeight);
	BottomButton.WinTop = (32.0/768.0*YMod)*(MinOptions+1);
	BottomButton.MyFont = class'UTLadderStub'.Static.GetBigFont(Root);
	if (NumOptions > MinOptions+OptionOffset)
		BottomButton.bDisabled = False;
	else
		BottomButton.bDisabled = True;
}


///////////////////////////////////////
// Paint
///////////////////////////////////////

function Paint(Canvas C, float X, float Y)
{
	local int i;

	Super.Paint(C, X, Y);

	// Text
	for(i=0; i<NumOptions; i++)
	{
		OptionButtons[i].FadeFactor = FadeFactor/100;
	}
}

 
///////////////////////////////////////
// Notify
///////////////////////////////////////

function Notify(UWindowWindow B, byte E)
{
	local int i;
	local s_player PL;
	local int price;

	switch (E)
	{
		case DE_DoubleClick:
		case DE_Click:
			GetPlayerOwner().PlaySound(sound'SpeechWindowClick', SLOT_Interact);
			for (i=0; i<NumOptions; i++)
			{
				if (B == OptionButtons[i])
				{
					//log("Giving Weapon *******************");
					PL = s_Player(GetPlayerOwner());
					if (PL != None)
					{	
						//log("Weapon Bought");
						PL.BuyWeapon(WepCode[i]);				
					}
					else
						log("not an s_Player");
				}
			}
			if (B == TopButton)
			{
				if (NumOptions > 8)
				{
					if (OptionOffset > 0)
						OptionOffset--;
				}
			}
			if (B == BottomButton)
			{
				if (NumOptions > 8)
				{
					if (NumOptions - OptionOffset > 8)
						OptionOffset++;
				}
			}
			break;
	}
}


///////////////////////////////////////
// defaultproperties
///////////////////////////////////////

defaultproperties
{
     PreName="$"
     TopTexture=Texture'TODatas.Skins.AW_OrdersTop2'
}
