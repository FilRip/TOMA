...
var string 	LoginMsg, LogoutMsg, FailedMsg, BanMsg;
var	int		TestLevel;
var bool	CWMode;
...
function	SALogOut(PlayerPawn Player)
{
	local	int	i;
	...
	{
		if (Admins[i].SAL < SilentLogin || CWMode) {
            NotifyRest(0, Player, Player.PlayerReplicationInfo.PlayerName@LogoutMsg@Admins[i].SAL);
            NotifyPlayer(1, Player, Player.PlayerReplicationInfo.PlayerName@LogoutMsg@Admins[i].SAL);
   		    XLog(Player.PlayerReplicationInfo.PlayerName@LogoutMsg@Admins[i].SAL);
        } else {
           	NotifyPlayer(1, Player, "You"@LogoutMsg@Admins[i].SAL@"silently");
			XLog(Player.PlayerReplicationInfo.PlayerName@LogoutMsg@Admins[i].SAL@"silently");
		}
