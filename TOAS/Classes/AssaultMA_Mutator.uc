class AssaultMA_Mutator extends Mutator config;
...
{
	if ( SpawnClass == Class'AssaultPlayer' )
	{
		SpawnClass=Class'AssaultMA_Player';
	}
	if ( NextMutator != None )
	{
...
function Tick (float Delta)
{
	...
	local AssaultMA_Player MA_ViewTarget;
	local bool bThunder;
	local int Count,Num;
	...
	for( Pawn=Level.PawnList; Pawn!=None; Pawn=Pawn.NextPawn )
	{
		...
		*/
		...
		if ( AssaultMA_Player(Pawn) != None)
		{
			if (!AssaultMA_Player(Pawn).bMALoggedIn)
				NewPlayerLogin(Pawn);

			MA_Player=AssaultMA_Player(Pawn);

			if ( AssaultMA_Player(MA_Player.ViewTarget ) != None)
			{
				MA_ViewTarget=AssaultMA_Player(MA_Player.ViewTarget);
		...
		if ( bThunder )
		{
			...
			if ( AssaultMA_Player(Pawn) != None )
			{
				AssaultMA_Player(Pawn).MAplaySound(Num);
			}
		}
	}
...
{
	local AssaultMA_ReplicationInfo MA_ReplicationInfo;
	local AssaultMA_Player MA_Player;
	...
	foreach AllActors(Class'AssaultMA_ReplicationInfo',MA_ReplicationInfo)
	{
	...
	MA_ReplicationInfo=Spawn(Class'AssaultMA_ReplicationInfo',Pawn,,Pawn.Location);
